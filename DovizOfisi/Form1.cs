...
namespace DovizOfisi
{
    public partial class Form1 : Form
    {
        ...
        private string connectionString = "Data Source=DESKTOP-TFCL3EH;Initial Catalog=DBSECIMPROJE;Integrated Security=True";
        ...
        private void btnyap_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string query = "INSERT INTO DovizTablosu (Tarih, TLDeger, DolarDeger, EuroDeger) VALUES (GETDATE(), @TLDeger, @DolarDeger, @EuroDeger)";

                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@TLDeger", tutar);
                    command.Parameters.AddWithValue("@DolarDeger", 0);
                    command.Parameters.AddWithValue("@EuroDeger", 0);
        ...
        private void btnyap2_Click(object sender, EventArgs e)
        {
            double kur = Convert.ToDouble(txtkur.Text);
            int miktar = Convert.ToInt32(txtmiktar.Text);
            int tutar = Convert.ToInt32(kur / miktar);
            txttutar.Text = tutar.ToString();
            double kalan;
            kalan = miktar % kur;
